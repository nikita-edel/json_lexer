...
#define JSON_TOKEN_STR(x) \
    (((x) < 0 || (x) >= JSON_TOKEN__FILLER_LAST) ? "UNKNOWN" : JSON__TOKEN_STRINGS[x])

#define JSON_LEX_ERR_STR(x) \
    (((x) < 0 || (x) >= JSON_LEX__FILLER_LAST) ? "ERR_UNKNOWN" : JSON__LEX_ERR_STRINGS[x])
...
void json_lexer_init(json_lexer_t* lex, const char* input_stream, const char* input_end);
