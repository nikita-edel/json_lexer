...
void json_lexer_init(json_lexer_t* lex, const char* input_start,
...
LEX_STIN JSON_LEX_ERR lexer_number(json_lexer_t* lex) {
	...
	if (st != JSON_LEX_OK && st != JSON_LEX_EOF) return st;
...
LEX_STIN JSON_LEX_ERR lexer_string(json_lexer_t* lex) {
	...
	size_t strLen = (size_t)(lex->parse_point - lex->token.start);
	if (strLen >= UINT32_MAX) return JSON_LEX_ERR_STR_TOO_LONG;
...
LEX_STIN JSON_LEX_ERR lexer_symbol(json_lexer_t* lex) {
	...
	switch (c) {
		case '{':
			lex->token.type = JSON_TOKEN_LBRACE;
			break;
		case '}':
			lex->token.type = JSON_TOKEN_RBRACE;
			break;
		case '[':
			lex->token.type = JSON_TOKEN_LBRACKET;
			break;
		case ']':
			lex->token.type = JSON_TOKEN_RBRACKET;
			break;
		case ':':
			lex->token.type = JSON_TOKEN_COLON;
			break;
		case ',':
			lex->token.type = JSON_TOKEN_COMMA;
			break;
		default:
			return JSON_LEX_ERR_UNEXPECTED_SYMBOL;
	}
...
LEX_STIN char lex_char(const json_lexer_t* lex) { return *lex->parse_point; }
...
LEX_STIN JSON_LEX_ERR init_token(json_lexer_t* lex) {
	...
	if (lex->line >= UINT32_MAX) return JSON_LEX_ERR_LINE_OVERFLOW;

	lex->token.line = (uint32_t)lex->line;

	if (lex->col >= UINT32_MAX) return JSON_LEX_ERR_COL_OVERFLOW;
...
LEX_STIN bool is_digit(char c) { return c >= '0' && c <= '9'; }

LEX_STIN bool is_number_start(char c) { return is_digit(c) || c == '-'; }

LEX_STIN bool is_string_start(char c) { return c == '"'; }

LEX_STIN bool is_control_char(char c) { return c <= 0x1F; }
